...
export const SELECTORS = {
  ...
  DEVICES: {
    ...
    AUDIO_INPUT_DEVICES: '[data-testid="audio-input-devices"]',
    AUDIO_OUTPUT_DEVICES: '[data-testid="audio-output-devices"]',
...
export function getDTMFSelector(digit: string): string {
  const key =
    `DTMF_${digit === '*' ? 'STAR' : digit === '#' ? 'HASH' : digit}` as keyof typeof SELECTORS.DTMF
  return SELECTORS.DTMF[key] || `[data-testid="dtmf-${digit}"]`
}
...
export const TEST_DATA = {
  ...
  // Phone numbers
  PHONE_NUMBERS: {
    VALID: 'sip:destination@example.com',
    VALID_2: '+15551234567',
    INVALID: 'abc',
  },

  // Legacy phone number constants for backward compatibility
