...
test.describe('Validation Errors', () => {
  ...
  test('should show error when SIP URI is empty', async ({ page }) => {
    ...
    // Clear SIP URI
    await page.fill(SELECTORS.SETTINGS.SIP_URI_INPUT, '')
    ...
    await expect(page.locator(SELECTORS.SETTINGS.VALIDATION_ERROR)).toContainText(/required/i)
  })
  ...
  test('should show error when password is empty', async ({ page }) => {
    ...
    await page.fill(SELECTORS.SETTINGS.PASSWORD_INPUT, '')
    await page.fill(SELECTORS.SETTINGS.SERVER_URI_INPUT, TEST_DATA.VALID_WS_URI)
  ...
  test('should show error when server URI is empty', async ({ page }) => {
    ...
    await page.fill(SELECTORS.SETTINGS.SIP_URI_INPUT, TEST_DATA.VALID_SIP_URI)
    await page.fill(SELECTORS.SETTINGS.PASSWORD_INPUT, TEST_DATA.VALID_PASSWORD)
    await page.fill(SELECTORS.SETTINGS.SERVER_URI_INPUT, '')
  ...
  test('should show error for invalid SIP URI format', async ({ page }) => {
    ...
    await page.fill(SELECTORS.SETTINGS.SIP_URI_INPUT, TEST_DATA.INVALID_SIP_URI_NO_SCHEME)
    await page.fill(SELECTORS.SETTINGS.PASSWORD_INPUT, TEST_DATA.VALID_PASSWORD)
    await page.fill(SELECTORS.SETTINGS.SERVER_URI_INPUT, TEST_DATA.VALID_WS_URI)
  ...
  test('should show error for invalid WebSocket URI format', async ({ page }) => {
    ...
    await page.fill(SELECTORS.SETTINGS.SERVER_URI_INPUT, TEST_DATA.INVALID_WS_URI_HTTP)
  ...
  test('should clear validation error when correcting input', async ({ page }) => {
    ...
    await expect(page.locator(SELECTORS.SETTINGS.VALIDATION_ERROR)).not.toBeVisible()
    await expect(page.locator(SELECTORS.SETTINGS.SETTINGS_SAVED_MESSAGE)).toBeVisible()
  })
})
...
test.describe('Connection Errors', () => {
  ...
  test('should disable call button when not connected', async ({ page }) => {
    ...
    await page.fill(SELECTORS.DIALPAD.NUMBER_INPUT, TEST_DATA.PHONE_NUMBER_1)
  ...
  test('should not show disconnect button when not connected', async ({ page }) => {
    await expect(page.locator(SELECTORS.CONNECTION.DISCONNECT_BUTTON)).not.toBeVisible()
    await expect(page.locator(SELECTORS.CONNECTION.CONNECT_BUTTON)).toBeVisible()
  })

  test('should show proper status when disconnected', async ({ page }) => {
    const status = page.locator(SELECTORS.STATUS.CONNECTION_STATUS)
...
test.describe('Call Control Errors', () => {
  ...
  test('should not show call controls when no active call', async ({ page }) => {
    await expect(page.locator(SELECTORS.CALL_CONTROLS.ACTIVE_CALL)).not.toBeVisible()
    await expect(page.locator(SELECTORS.CALL_CONTROLS.HANGUP_BUTTON)).not.toBeVisible()
...
test.describe('UI Edge Cases', () => {
  ...
  test('should handle rapid device settings toggling', async ({ page }) => {
    const deviceButton = page.locator(SELECTORS.DEVICES.DEVICE_SETTINGS_BUTTON)
  ...
  test('should handle rapid call history toggling', async ({ page }) => {
    const historyButton = page.locator(SELECTORS.HISTORY.CALL_HISTORY_BUTTON)
    const historyPanel = page.locator(SELECTORS.HISTORY.CALL_HISTORY_PANEL)
  ...
  test('should show empty call history initially', async ({ page }) => {
    await page.click(SELECTORS.HISTORY.CALL_HISTORY_BUTTON)

    // History panel should be visible
    await expect(page.locator(SELECTORS.HISTORY.CALL_HISTORY_PANEL)).toBeVisible()

    // But no history entries
    await expect(page.locator(SELECTORS.HISTORY.HISTORY_ENTRY)).toHaveCount(0)
  })
  ...
  test('should use password input type for password field', async ({ page }) => {
    ...
    const passwordInput = page.locator(SELECTORS.SETTINGS.PASSWORD_INPUT)
...
test.describe('Device Management Errors', () => {
  ...
  test('should display available audio devices', async ({ page }) => {
    ...
    const outputSelect = page.locator(SELECTORS.DEVICES.AUDIO_OUTPUT_SELECT)
  ...
  test('should show feedback when changing devices', async ({ page }) => {
    ...
    await expect(page.locator(SELECTORS.DEVICES.DEVICE_CHANGED_MESSAGE)).toBeVisible()

    // Message should disappear after timeout
    await page.waitForTimeout(3500)
    await expect(page.locator(SELECTORS.DEVICES.DEVICE_CHANGED_MESSAGE)).not.toBeVisible()
  })
})
...
test.describe('Responsive Design', () => {
  ...
  test('should work on mobile portrait (375x667)', async ({ page }) => {
    ...
    await expect(page.locator(SELECTORS.STATUS.CONNECTION_STATUS)).toBeVisible()
  })
